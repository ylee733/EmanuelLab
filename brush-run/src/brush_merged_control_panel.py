...
DEFAULT_SETTINGS = {
    ...
    '2p_trigger': '/Dev2/port0/line4',
    'laser': '/Dev2/port0/line5',
    'camera_trigger': '/Dev2/port0/line6',
...
def create_settings_window(settings):
    ...
    layout = [
        ...
        [TextLabel('camera_trigger'), sg.Input(key='-CAMERA TRIGGER-')],
...
def createLaserSettingsWindow():
    ...
    layout = [
        ...
        [sg.Text('DC probability:', size=(15,1)), sg.Input(default_text='0.7', key='-DCTrials-')],
        [sg.Text('FS1 probability:', size=(15,1)), sg.Input(default_text='0.1', key='-FS1Trials-')],
        [sg.Text('HS1 probability:', size=(15,1)), sg.Input(default_text='0.1', key='-HS1Trials-')],
        [sg.Text('PPC probability:', size=(15,1)), sg.Input(default_text='0.1', key='-PPCTrials-')],
...
def setupDaq(settings, taskParameters, setup='task'):
    ...
    if setup == 'task':
        ...
        do_task.do_channels.add_do_chan(settings['camera_trigger'], name_to_assign_to_lines='camera_trigger')
...
def runOperantTrial(di_task, ao_task, do_task, taskParameters, settings, trialIndex, totalTrials, probe=False):
    ...
    num_do_channels = 6 if not taskParameters.get('laserOutput', False) else 7
    ...
    if taskParameters.get('laserOutput', False):
        probabilities = [
            taskParameters['DCTrials'],
            taskParameters['FS1Trials'],
            taskParameters['HS1Trials'],
            taskParameters['PPCTrials']
        ]
        total_prob = sum(probabilities)
        normalized_probabilities = [p / total_prob for p in probabilities]
        regionID = np.random.choice([0, 1, 2, 3], p=normalized_probabilities)
        if regionID == 0:  # DCTrials
            print('stimulating Control')
            laserLoc = 'control'
            x = taskParameters['x_DC'] + taskParameters['x_Ref']
            y = taskParameters['y_DC'] + taskParameters['y_Ref']
        elif regionID == 1:  # FS1Trials
            print('stimulating FS1')
            laserLoc = 'FS1'
            x = taskParameters['x_FS1'] + taskParameters['x_Ref']
            y = taskParameters['y_FS1'] + taskParameters['y_Ref']
        elif regionID == 2:  # HS1Trials
            print('stimulating HS1')
            laserLoc = 'HS1'
            x = taskParameters['x_HS1'] + taskParameters['x_Ref']
            y = taskParameters['y_HS1'] + taskParameters['y_Ref']
        elif regionID == 3:  # PPCTrials
            print('stimulating PPC')
            laserLoc = 'PPC'
            x = taskParameters['x_PPC'] + taskParameters['x_Ref']
            y = taskParameters['y_PPC'] + taskParameters['y_Ref']
        x *= taskParameters['V_div_mm']
        y *= taskParameters['V_div_mm']
        ao_out[0,1:-1] = x
        ao_out[1,1:-1] = y
        ...
        print('samples between first two laser pulses,', laserStart_samples[1] - laserStart_samples[0])
        if taskParameters.get('laserOutput', False):
            for start_end in zip(laserStart_samples, laserEnd_samples):
                do_out[6, start_end[0]:start_end[1]] = True
...
def the_gui():
    ...
    left_column = [
        ...
        [sg.Text('Save Path', size=(textWidth,1)), sg.Input(os.path.normpath(r'E:\DIRECTION\TEMP_BEHAVIOR'), size=(20,1), key='-SavePath-'),
